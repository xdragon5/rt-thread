...
* Log: 	 2018/09/23, Create this function by zhuangwei
 ******************************************************************************/
void gpio_set_irq_mode(gpio_pin_t pin, uint8_t mode)
{
    uint8_t exti_num;
	...
	{
		PMU_ExintEdge &= ~(((uint32_t)0x01)<<exti_num);
	}
	else
	{
		PMU_ExintEdge |= (((uint32_t)0x01)<<exti_num);
...
* Log: 	 2018/09/23, Create this function by zhuangwei
 ******************************************************************************/
void gpio_set_irq_pol(gpio_pin_t pin, uint8_t pol)
{
    uint8_t exti_num;
	...
	{
		PMU_ExintPol &= ~(((uint32_t)0x01)<<exti_num);
	}
	else
	{
		PMU_ExintPol |= (((uint32_t)0x01)<<exti_num);
...
* Log: 	 2018/09/23, Create this function by zhuangwei
 ******************************************************************************/
void gpio_remap(gpio_pin_t pin, gpio_func_t func)
{
	uint8_t gpio_group = pin/16;
	uint8_t gpio_offset = (pin%16)*2;
